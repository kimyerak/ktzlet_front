...
export default function TestSetup({ onNext, onBack, initialConfig }) {
  const [testConfig, setTestConfig] = useState(() => {
    // 기본 날짜 설정 (2025.8.1 ~ 2025.8.31)
    const startDate = new Date('2025-08-01T00:00');
    const endDate = new Date('2025-08-31T23:59');
    
    return {
      title: '',
      numofquestion: 10,
      open_at: startDate.toISOString().slice(0, 16), // 2025-08-01T00:00
      close_at: endDate.toISOString().slice(0, 16), // 2025-08-31T23:59
      time_limit_sec: 3600, // 60분을 초 단위로
      target_score: 70,
      ...initialConfig,
      open_at: initialConfig?.open_at || startDate.toISOString().slice(0, 16),
      close_at: initialConfig?.close_at || endDate.toISOString().slice(0, 16)
    };
  });
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-blue-200">
          ...
          <div className="flex justify-between mt-12">
            ...
            >
              {initialConfig?.isAutoGenerated ? '🎉 완료!' : '다음 단계 → 단어 선택하기 📚'}
            </button>
          </div>
        </div>
      </div>
